...
function prepareWords(words) {
    // randomize order of words and filter only those that are convertable, then return them in lowercase
    words = words.sort(() => Math.random() - 0.5).sort(() => Math.random() - 0.5); // I know, it's bad
    return words.filter(allConvertable)
}


function translate(word) {
    word = bezogonki(word.toLowerCase());
    ...
    reactions: [
        ...
        function next($, store) {
            if (store.advanceSoon) {
                ...
                setTimeout(async () => {
                    try {
                        const newQ = makeQuestion(store.words.pop());
                        if (newQ && newQ.q) {
                            store.question = newQ;
                            ...
                            store.words = prepareWords(WORDS)
                        }
                    } catch (error) {
                        console.error(error)
                        store.error = error.message
